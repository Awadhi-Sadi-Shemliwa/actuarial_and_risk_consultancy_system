# Deploying ARC to production (system.arc.co.tz)
...
Wait a few minutes, then confirm: `ping system.arc.co.tz` should resolve
to your VPS IP.
...
# Clone the repo
sudo mkdir -p /opt/arc && sudo chown $USER:$USER /opt/arc
git clone https://github.com/Awadhi-Sadi-Shemliwa/actuarial_and_risk_consultancy_system.git /opt/arc
cd /opt/arc
...
| `DB_NAME`                | Database name, e.g. `arc_business`          |
...
```bash
sudo cp deploy/nginx/system.arc.co.tz.conf \
        /etc/nginx/sites-available/system.arc.co.tz
sudo ln -s /etc/nginx/sites-available/system.arc.co.tz \
           /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx

# Free TLS cert (Certbot adds the 443 block + HTTP->HTTPS redirect automatically)
sudo certbot --nginx -d system.arc.co.tz
```

Visit **https://system.arc.co.tz** and log in with your
